...
download_xui(){
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/toxicare/x-ui-english/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/') || last_version=$(curl -sm8 https://raw.githubusercontent.com/toxicare/x-ui-english/main/config/version >/dev/null 2>&1)
        ...
        last_version=$1
        url="https://github.com/toxicare/x-ui-english/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz"
...
install_xui() {
    ...
    echo -e "Niduka Akalanka   - https://github.com/toxicare"
    echo -e "--------------------------------------------------------------------------------"
    show_login_info
    echo -e ""
