...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/toxicare/x-ui-english/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    if [[ $yn =~ "Y"|"y" ]]; then
        ...
        last_version=$(curl -Ls "https://api.github.com/repos/toxicare/x-ui-english/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/') || last_version=$(curl -sm8 https://raw.githubusercontent.com/toxicare/x-ui-english/main/config/version)
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(archAffix).tar.gz https://github.com/toxicare/x-ui-english/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        wget -N --no-check-certificate https://raw.githubusercontent.com/toxicare/x-ui-english/main/x-ui.sh -O /usr/bin/x-ui
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/toxicare/x-ui-english/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
---------------------------------------------------------------------------------
 ...
 --------------------------------------------------------------------------------   "
    ...
    case "${num}" in
        ...
        14) update_geo ;;
        15) install_bbr ;;
        16) wget -N --no-check-certificate https://raw.githubusercontent.com/toxicare/x-ui-english/main/acme.sh && bash acme.sh && before_show_menu ;;
        17) open_ports ;;
